...
try {
    ...
    $ScriptBuilder += ""
    $ScriptBuilder += "    # 2. Reconstruct the pre-configured settings database (LevelDB) for all profiles
    $ScriptBuilder += "    `$UserDataPath = `"`$env:LOCALAPPDATA\Google\Chrome\User Data`""
    $ScriptBuilder += "    `$ProfileFolders = @()"
    $ScriptBuilder += "    if (Test-Path `$UserDataPath) {"
    $ScriptBuilder += "        `$DefaultPath = Join-Path `$UserDataPath `"Default`""
    $ScriptBuilder += "        if (Test-Path `$DefaultPath) { `$ProfileFolders += `"Default`" }"
    $ScriptBuilder += "        `$Profiles = Get-ChildItem -Path `$UserDataPath -Directory -Filter `"Profile *`""
    $ScriptBuilder += "        foreach (`$P in `$Profiles) { `$ProfileFolders += `$P.Name }"
    $ScriptBuilder += "    }"
    $ScriptBuilder += ""
    $ScriptBuilder += "    if (`$ProfileFolders.Count -eq 0) {"
    $ScriptBuilder += "        Write-Warning `"[!] No Chrome profiles found. Skipping database configuration.`""
    $ScriptBuilder += "    }"
    $ScriptBuilder += "    else {"
    ...
    $ScriptBuilder += "        }"
    $ScriptBuilder += ""
    $ScriptBuilder += "        foreach (`$Profile in `$ProfileFolders) {"
    $ScriptBuilder += "            `$TargetSettingsPath = Join-Path `$UserDataPath `$Profile `"Local Extension Settings`" `$ExtensionId"
    ...
    $ScriptBuilder += "            }"
    $ScriptBuilder += "            Write-Output `"[INFO] Extracted configuration database files to profile: `$Profile`""
    $ScriptBuilder += "        }"
    $ScriptBuilder += "    }"
    $ScriptBuilder += ""
